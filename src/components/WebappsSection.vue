  ...
  <section id="webapp" class="page-section webapp">
    <div class="container">
      ...
      <div class="row g-4 webapp-block reviewer-all-row">
        <div class="col-lg-6">
          <div class="webapp-category-card reviewer-block h-100">
            <h3 class="webapp-subsection-title text-uppercase text-secondary">
              eBird Reviewer Apps
            </h3>
            <div class="row justify-content-center reviewer-row">
              <a
                v-for="app in reviewerApps"
                :key="app.id"
                :href="app.url"
                :aria-label="`Open ${app.title}`"
                class="col-sm-6 col-lg-12 col-xl-6 mb-4 text-decoration-none webapp-card-link"
              >
                <div class="webapp-item reviewer-item mx-auto">
                  <img
                    v-if="app.previewImage"
                    :src="app.previewImage"
                    :alt="app.alt || `${app.title} preview`"
                    class="img-fluid mb-2 reviewer-image"
                  />
                  <h4>{{ app.title }}</h4>
                  <p>{{ app.description }}</p>
                </div>
...
const reviewerApps = computed(() => appsFromIds(props.reviewerAppIds));

const allApps = computed(() => {
  const seen = new Set();
  const blockedIds = new Set([...props.featuredAppIds, ...props.reviewerAppIds]);
